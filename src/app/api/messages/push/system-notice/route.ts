...
export async function POST(request: NextRequest) {
  try {
    ...
    // 记录推送日志
    const logQuery = `
      INSERT INTO push_logs (type, title, content, created_by, created_at)
      VALUES (?, ?, ?, ?, NOW())
    `;
    ...
    // 调用实际的推送服务
    const pushResult = await sendSystemNoticePush(title, content);

    logger.info('系统通知推送完成', {
      user_id: user.id,
      title,
    ...
    logger.error('系统通知推送失败', { error: error instanceof Error ? error.message : String(error) });
