...
const titleMap: TitleMap = {
  ...
  '/app/config': 'APP基础配置',
  '/app/version': 'APP版本管理',
  '/app/recommend': 'APP推荐管理',
  '/app/announcement': '公告推送',
  '/app/notification': '通知推送',
  '/app/push-logs': '推送日志',
  '/app/push-config': '推送配置',
  '/app/refresh': 'APP刷新管理',
...
export default function DashboardLayout({
  ...
  return (
    ...
    >
      ...
      <header className="fixed top-0 right-0 h-[48px] bg-card border-b border-border z-[90] transition-all duration-300 left-0 md:left-[57px]">
        <div className="h-full flex items-center justify-between px-3 sm:px-4">
          <div className="flex items-center space-x-3">
            {/* 移动端显示logo，PC端不显示logo（因为侧边栏已有logo） */}
            <div className="md:hidden flex items-center justify-center w-8 h-8 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-slate-800 dark:to-slate-900 rounded-lg shadow-sm">
              <img 
                src="/logo.svg" 
                alt="Logo" 
                className="h-6 w-6"
            ...
            <h1 className="text-base sm:text-lg font-medium text-foreground">{pageTitle}</h1>
          </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              className="md:hidden p-2 hover:bg-muted rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-primary/20"
      ...
      {/* 移动端侧边栏 */}
      <div className={`
        fixed top-0 right-0 h-full w-[280px] bg-card shadow-xl z-[96] md:hidden
      ...
      `}>
        <div className="flex items-center justify-between p-4 border-b border-border bg-muted/50">
          ...
          <button
            onClick={() => setMobileMenuOpen(false)}
            className="p-1.5 hover:bg-muted rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-primary/20"
            aria-label="关闭菜单"
          >
            <X className="h-5 w-5 text-muted-foreground" />
          </button>
